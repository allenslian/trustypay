

using System.Threading.Tasks;
using TrustyPay.Core.Cryptography.Http.Service;

namespace TrustyPay.Core.Cryptography.Http.Fixtures.Service
{
    internal class FakeResourceProvider : IResourceProvider
    {
        private bool _incorrectPublicKey = false;

        private bool _incorrectPrivateKey = false;

        private bool _hasPermission = true;

        private bool _hasPermissionException = false;


        public FakeResourceProvider(bool incorrectPublicKey = false, bool incorrectPrivateKey = false, bool hasPermission = true, bool hasPermissionException = false)
        {
            _incorrectPublicKey = incorrectPublicKey;
            _incorrectPrivateKey = incorrectPrivateKey;
            _hasPermission = hasPermission;
            _hasPermissionException = hasPermissionException;
        }

        public Task<RSACryptoProvider.PublicKey> GetAppPublicKeyAsync(string appId, string apiKey) => Task.FromResult(RSAKeyFactory.ImportPublicKeyFromBase64String(
                "",
                _incorrectPublicKey ? RSACryptoProvider.PublicKeyFormat.Pkcs1 : RSACryptoProvider.PublicKeyFormat.X509
            ));

        public RSACryptoProvider.PrivateKey GetPlatformPrivateKey()
        {
            return RSAKeyFactory.ImportPrivateKeyFromBase64String(
                "",
                _incorrectPrivateKey ? RSACryptoProvider.PrivateKeyFormat.Pkcs8 : RSACryptoProvider.PrivateKeyFormat.Pkcs1
            );
        }

        public Task<bool> HasPermissionByApiKeyAsync(string apiKey, string path, string method)
        {
            if (_hasPermissionException)
            {
                throw new System.Exception("Permission error!!!");
            }
            return Task.FromResult(_hasPermission);
        }
    }
}